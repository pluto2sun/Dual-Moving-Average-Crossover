...
from url.url_formats import (buff_index_json_fmt, c5_search_page_fmt,
                         igxe_search_page_fmt, steam_item_page_fmt,
                         uuyp_search_page_fmt)
from utils.utils import (asian_proxies, calculate_after_fee, default_header,
...
# ==== load buff_cookie ====
with open('../cookies/buff_cookie.txt', 'r', encoding='utf-8') as bc:
    buff_cookie = bc.read().strip()
    assert 'session' in buff_cookie

# ==== load c5_cookie ====
with open('../cookies/c5_cookie.txt', 'r', encoding='utf-8') as cc:
    c5_cookie = cc.read().strip()
    assert 'C5Login' in c5_cookie

# ==== load uuyp_cookie ====
with open('../cookies/uuyp_cookie.txt', 'r', encoding='utf-8') as uc:
    uuyp_cookie = uc.read().strip()
    assert 'Bearer' in uuyp_cookie

# ==== load igxe_cookie ====
# with open('../cookies/igxe_cookie.txt', 'r', encoding='utf-8') as ic:
#     igxe_cookie = ic.read().strip()
#     assert 'Bearer' in igxe_cookie

# ==== init ====
headers = default_header.copy()
headers['cookies'] = buff_cookie + ';' + c5_cookie
headers['authorization'] = uuyp_cookie
platforms = ['buff', 'c5', 'uuyp', 'igxe']
game_info = [
    {'game': 'csgo',  'appid':730},
...
def get_c5_id(game:str, name:str):
    r = requests.get(c5_search_page_fmt.format(name=name, game=game), headers=headers, timeout=20)
